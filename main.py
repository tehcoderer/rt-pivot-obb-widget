...
@register_widget(
    {
        ...
        "gridData": {"w": 24, "h": 16},
        "data": {
            "wsRowIdColumns": ["Ticker", "Fund", "Strategy", "PM", "Portfolio"],
            "table": {
                "enableCharts": True,
                "showAll": False,
                "columnsDefs": [
                    ...
                    },
                    {
                        ...
                        "cellDataType": "number",
                        "formatterFn": "int",
                        "renderFn": ["showCellChange", "columnColor"],
                    },
                    {
                        "field": "MarketValue",
    ...
    try:
        ...
        if minMarketValue > 0:
            filtered_data = filtered_data[
                filtered_data["MarketValue"] >= minMarketValue
            ]
        ...
        if len(group_cols) > 0:
            ...
            grouped_data = (
                filtered_data.groupby(group_cols).agg(agg_funcs).reset_index()
            )
            grouped_data = grouped_data.rename(columns={"Ticker": "PositionCount"})

            # Add computed metrics
            grouped_data["AvgBeta"] = (
                filtered_data.groupby(group_cols)["Beta"].mean().values
            )
            grouped_data["AvgADV"] = (
                filtered_data.groupby(group_cols)["ADV%"].mean().values
            )
            ...
            if not showDetails:
                ...
                detailed_only_cols = set(detailed_data.columns) - set(
                    grouped_data.columns
                )
                grouped_only_cols = set(grouped_data.columns) - set(
                ...
                result_data = pd.concat(
                    [grouped_data, detailed_data], ignore_index=True
                )
        else:
        ...
        result_data = result_data.replace(
            [float("inf"), float("-inf")], [999999999, -999999999]
        )  # Replace infinity
...
def portfolio_tree():
    ...
    try:
        ...
        grouped = (
            data.groupby(["Fund", "Strategy", "PM", "Portfolio"])
            .agg(
                {
                    "MarketValue": "sum",
                    "PnL_Day": "sum",
            ...
            .rename(columns={"Ticker": "PositionCount"})
            .reset_index()
        )
...
def get_live_position_data(symbols: List[str]):
    ...
    global LIVE_WS_DATA
    data = portfolio_generator.get_current_data(symbols)
    if data is None or len(data) == 0:
        data = portfolio_generator.generate_portfolio_data(
            1000
        )  # Smaller dataset for live updates
        portfolio_generator.set_current_data(data)

    # Filter data to requested symbols if any, otherwise get top positions
    if symbols and len(symbols) > 0:
        filtered_data = data[data["Ticker"].isin(symbols)].copy()
    else:
        # Get top 20 positions by market value for live demo
        filtered_data = data.nlargest(20, "MarketValue").copy()
            ...
            "table": {
                ...
                "columnsDefs": [
                    ...
                    },
                    {
                        ...
                        "renderFnParams": {"colorValueKey": "PnL_Day"},
                        "formatterFn": "int",
                    },
                    {
                        "field": "Quantity",
...
async def live_portfolio_positions(symbols: str = "AAPL,GOOGL,MSFT,AMZN,TSLA"):
        ...
        try:
            async for data in ws.iter_json():
                if symbols := data.get("params", {}).get("symbols"):
                    ...
                    logger.info(
                        f"WebSocket subscribed to symbols: {subscribed_symbols}"
                    )
        ...
        try:
            while websocket.client_state != WebSocketState.DISCONNECTED:
                if subscribed_symbols:
                    ...
                    # Update 3 symbols per cycle
                    choice_symbols: list[str] = np.random.choice(
                        current_symbols,
                        size=min(3, len(current_symbols)),
                        replace=False,
                    ).tolist()
                    symbols_data = get_live_position_data(choice_symbols)

                    # Send updates for a few symbols at a time
                    for symbol in choice_symbols:
                        try:
                            row = symbols_data[symbols_data["Ticker"] == symbol]
                            if not row.empty:
                                row = row.iloc[0]
                                update = {
                                    "Ticker": symbol,
                    ...
                    updates = portfolio_generator.get_random_updates(20)
                ...
                await asyncio.sleep(np.random.uniform(0.1, 0.4))  # Pause between cycles
