...
class PortfolioDataGenerator:
    ...
    def _generate_tickers_data(self, tickers: List[str]) -> pd.DataFrame:
        ...
        if self.current_data is not None:
            df = pd.concat([self.current_data, df], ignore_index=True)
    ...
    def create_hierarchical_data(self, df: pd.DataFrame) -> List[Dict[str, Any]]:
        ...
        for _, row in df.iterrows():
            ...
            if (
                portfolio
                not in hierarchy[fund]["children"][strategy]["children"][pm]["children"]
            ):
                hierarchy[fund]["children"][strategy]["children"][pm]["children"][
                    portfolio
                ] = {
                    "Fund": fund,
                    "Strategy": strategy,
            ...
            for metric in ["MarketValue", "PnL_Day", "PnL_MTD", "VaR_95"]:
                ...
                hierarchy[fund]["children"][strategy]["aggregates"][metric] += row[
                    metric
                ]
                hierarchy[fund]["children"][strategy]["children"][pm]["aggregates"][
                    metric
                ] += row[metric]
                hierarchy[fund]["children"][strategy]["children"][pm]["children"][
                    portfolio
                ]["aggregates"][metric] += row[metric]
    ...
    def run(self, interval: float = 1.0):
        ...
        asyncio.run(self._update_loop(interval))
    ...
    def start_real_time_updates(self, update_interval: float = 1.0):
        ...
        update_thread = threading.Thread(
            target=self.run, args=(update_interval,), daemon=True
        )
        update_thread.start()
        with self.lock:
    ...
    def get_random_updates(self, count: int = 100) -> list[Dict[str, Any]]:
        """Get a DataFrame of random updates to simulate real-time changes"""
        with self.lock:
            if self.current_data is None:
                return []

            update_count = min(count, len(self.current_data))
            update_indices = np.random.choice(
                len(self.current_data), update_count, replace=False
            )

            updates = []
            for idx in update_indices:
                current_price = self.current_data.loc[idx, "Price"]
                price_change = np.random.normal(
                    0, current_price * 0.002
                )  # 0.2% volatility
                new_price = max(0.01, current_price + price_change)

                quantity = self.current_data.loc[idx, "Quantity"]
                old_market_value = self.current_data.loc[idx, "MarketValue"]
                new_market_value = quantity * new_price

                update_row = {
                    "Ticker": self.current_data.loc[idx, "Ticker"],
                    "Price": round(new_price, 2),
                    "Fund": self.current_data.loc[idx, "Fund"],
                    "Strategy": self.current_data.loc[idx, "Strategy"],
                    "PM": self.current_data.loc[idx, "PM"],
                    "Portfolio": self.current_data.loc[idx, "Portfolio"],
                    "MarketValue": round(new_market_value, 2),
                    "PnL_Day": round(
                        self.current_data.loc[idx, "PnL_Day"]
                        + (new_market_value - old_market_value),
                        2,
                    ),
                    "VaR_95": round(
                        self.current_data.loc[idx, "VaR_95"]
                        * (1 + np.random.normal(0, 0.01)),
                        2,
                    ),
                    "Beta": round(
                        self.current_data.loc[idx, "Beta"] + np.random.normal(0, 0.005),
                        4,
                    ),
                    "Volatility": round(
                        self.current_data.loc[idx, "Volatility"]
    ...
    def get_current_data(self, tickers: List[str] | None = None) -> pd.DataFrame:
        """Get current data with thread safety"""
        with self.lock:
            # get missing tickers
            if tickers and self.current_data is not None:
                missing_tickers = [
                    t for t in tickers if t not in self.current_data["Ticker"].values
                ]
                current_data = self.current_data
                if missing_tickers:
                    current_data = self._generate_tickers_data(missing_tickers)

                return current_data.copy()

            if tickers and self.current_data is None:
                new_data = self._generate_tickers_data(tickers)
